...
| 未着手 | 実測に基づく調整（閾値・件数上限の再検討 → TASK-028 / TASK-035） |
...
| TASK-035 | `OQ` | 閾値（現在5）の感度を検証する | [ADR-003 §OQ-002](04_decisions/ADR-003_triage_model_selection.md#未解決事項-open-questions) | t_inoue | 2026-08-23 | `open` | モデルより閾値のほうが投入件数に効くレバーの可能性。TASK-023 と一体で検証する。**2026-08-03 追記:** 情報源を 12 → 58 件へ拡大した（設計・FP・処理系の個人ブログが中心）。**領域が広がったため既存のスコア分布は参考にならない** — トリアージ基準は元々データエンジニアリング領域を想定しており、新規ソースがどの帯に落ちるかは未知。初回実行後のスコア分布を見て、閾値と `profile.md` の基準の双方を再検討する。**閾値5と profile.md は「要合意」項目のため、実測を提示した上で運用者が判断する**。**2026-08-04 決定: 閾値は据え置き、`profile.md` の基準を書き直した。** 500件の分布（`0:102 1:15 2:89 3:76 4:66 5:6 6:45 7:34 8:65 9:2`）を `reason` ごと精査したところ、**低評価の主因は記事の質ではなく基準の設計**だった。(1) **129件が「関心領域外」で3点以下** — 基準が「データ基盤以外は3以下」と指示しており、処理系・型システムの記事を読むために追加した情報源（`scattered-thoughts` は186件中78件が0点）を基準自体が排除していた。(2) **5点に谷（4:66 → 5:6 → 6:45）** — 「判断材料が乏しい場合は 4〜5 を基準にする」という文言が**閾値ちょうどに曖昧なものを置いて**いた。対処として対象領域を「ソフトウェアエンジニアリング全般」へ広げ、材料不足は 0〜1 とした。**閾値の妥当性は書き直し後の分布を見てから再判断する**（領域が広がったぶん投入件数は増える見込み）。**2026-08-05 実測: 閾値5を据え置く。** 新基準で982件を評価した結果、閾値ごとの投入見込みは次のとおり（定常状態の新着を週50件と仮定した換算）。閾値5 → 週16件（率32%）/ 閾値6 → 週14件（28%）/ 閾値7 → 週10件（19%）。**閾値6にしても週2件しか減らない** — `final_score == 5` が43件（全体の4%）しかないため。その43件は「素点4 + weight+1」のハウツー記事が中心で、基準が言う「書き手の判断が残っている文章」からは外れる。**据え置いた理由:** 累積554件の投入は過去記事の総ざらいによるもので、定常状態の週16件は適量の可能性がある。**次の判断材料:** 定常状態の週次実行を3回以上観測してから再検討する。閾値7は要求定義の「週20本」想定を下回るが、実際に読める量としては現実的かもしれない（その場合 `#hot`（7以上）の定義と重なるため再定義が要る） |
