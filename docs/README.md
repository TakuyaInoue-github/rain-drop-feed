...
| TASK-035 | `OQ` | 閾値（現在5）の感度を検証する | [ADR-003 §OQ-002](04_decisions/ADR-003_triage_model_selection.md#未解決事項-open-questions) | t_inoue | 2026-08-23 | `open` | モデルより閾値のほうが投入件数に効くレバーの可能性。TASK-023 と一体で検証する。**2026-08-03 追記:** 情報源を 12 → 58 件へ拡大した（設計・FP・処理系の個人ブログが中心）。**領域が広がったため既存のスコア分布は参考にならない** — トリアージ基準は元々データエンジニアリング領域を想定しており、新規ソースがどの帯に落ちるかは未知。初回実行後のスコア分布を見て、閾値と `profile.md` の基準の双方を再検討する。**閾値5と profile.md は「要合意」項目のため、実測を提示した上で運用者が判断する** |
