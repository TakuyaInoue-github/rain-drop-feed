...
| TASK-115 | `OQ` | トリアージ判定を多軸化するか判断する（検討済み・当面見送り） | 実地の分布分析（2026-08-04） | t_inoue | 2026-10-31 | `wontfix` | 2026-08-04: 単一スコア（0-10）を `unscorable` / `scope` / `judgment` / `mechanism` の多軸へ分解する案を検討し、**当面見送った**。動機は「0点が『対象外』『材料不足』『低評価』を同じ値に潰しており、落ちた理由が読めない」こと。**見送りの理由:** (1) 実データの精査で、低評価の主因が基準の文言（領域の限定と「材料が乏しければ4〜5」）だと判明し、`profile.md` の書き直しだけで対処できる見込みが立った。(2) `scope` は情報源ごとの `weight` と重複し、投入先が単一コレクションのため配架の実体がない。(3) `judgment_markers` は合成規則が消費せず検証不能で、LLM が後付けで理由を並べる余地が大きい。(4) `mechanism` は合成表の1セルしか変えない。(5) 利用者1人・週数十件の規模に対して複雑すぎる（YAGNI）。**再検討の条件:** 書き直し後の分布でも落ちた理由が読めない、または「主張は弱いが仕組みの解説として価値がある」記事を体系的に取りこぼしていることが実測で確認された場合 |
