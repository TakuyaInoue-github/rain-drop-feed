...
> **2026-08-03 時点で本番稼働中。** GitHub Actions の週次実行（日曜 21:17 UTC）で
> フィード取得 → LLM トリアージ → Raindrop 投入まで動作しており、状態は orphan branch
> `state` に蓄積されている。SPEC 層は全6本を執筆済みで、実装はそれに追随している。
...
| 仕様 (SPEC-xxx) | `03_specs/` | `draft` | **全6本を執筆済み**（001〜006）。うち 001 / 002 / 003 / 004 / 006 はレビュー反映済み。SPEC-005 のみレビュー未実施 |
...
> 全6本を執筆済み。SPEC-002（状態管理）を先行して書き、独立レビュー（TASK-066）を
> 経てから残り5本を展開した。**実装はこの6本に追随している。**
...
| TASK-025 | `IMPL` | `ANTHROPIC_API_KEY` を GitHub Secrets に登録する | [ADR-001 §後続アクション](04_decisions/ADR-001_llm_invocation_method.md#後続アクション) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **登録済み。** `ANTHROPIC_API_KEY` / `RAINDROP_TOKEN` / `RAINDROP_COLLECTION_ID` の3種を GitHub Secrets へ登録し、`workflow_dispatch` の dry-run（170件・11分3秒）と本番実行の双方で注入を確認した |
| TASK-026 | `DOC` | `.ref/spec.md` の構成記述と ADR-001 の差異を SPEC 層に反映する | [ADR-001 §後続アクション](04_decisions/ADR-001_llm_invocation_method.md#後続アクション) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **反映済み。** SPEC-003 は Anthropic API の直接呼び出しを前提に記述されており（§5.1 の判別原則「Anthropic API から応答本体を受け取れたか」等）、`claude -p` を前提とした記述は残っていない。実装（`adapters/evaluate.py`）も ADR-001 に従い SDK 経由で呼んでいる。§7 の実測値の出典表記のみ `claude -p` 由来である旨が残るが、これは TASK-038（再測定の要否）の管轄 |
...
| TASK-049 | `DOC` | `.ref/spec.md` の `state.sqlite` 指定との差異を SPEC 層に反映する | [ADR-005 §後続アクション](04_decisions/ADR-005_state_file_format.md#後続アクション) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **反映済み。** SPEC-002 は「追記専用の JSONL（`state.jsonl`）」として記述されており（§1）、`state.sqlite` を前提とした記述は残っていない。ADR-005 の決定が SPEC 層へ正しく反映されている |
...
| TASK-061 | `DOC` | F-003 OQ-001 を `resolved` に更新し、AC-013 が ADR-005 の読み取り規則に従属することを明記する | [ADR-005 §後続アクション](04_decisions/ADR-005_state_file_format.md#後続アクション) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **反映済み。** F-003 OQ-001 は `resolved`（2026-07-26、追記専用 JSONL を採用）であり、AC-013 にも「『最新』の判定規則は ADR-005 OQ-001 による」と明記されている |
| TASK-051 | `IMPL` | 一意制約をアプリ側で担保する処理を SPEC に明示し、テスト観点に含める | [ADR-005 §後続アクション](04_decisions/ADR-005_state_file_format.md#後続アクション) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **完了。** SPEC-002 に一意化の責務を明記済み（フロー #3 / #15、§7「入力の一意化」）、テスト観点 T-036 も登録済み。実装も `pipeline._select_targets`（url をキーに重複排除）と `store.append_records`（同一実行内の二重追記を防止）の2箇所で担保し、`test_pipeline` / `test_store` / `test_ingest` に対応するテストがある |
...
| TASK-035 | `OQ` | 閾値（現在5）の感度を検証する | [ADR-003 §OQ-002](04_decisions/ADR-003_triage_model_selection.md#未解決事項-open-questions) | t_inoue | 2026-08-30 | `resolved` | モデルより閾値のほうが投入件数に効くレバーの可能性。TASK-023 と一体で検証する。**2026-08-03 追記:** 情報源を 12 → 58 件へ拡大した（設計・FP・処理系の個人ブログが中心）。**領域が広がったため既存のスコア分布は参考にならない** — トリアージ基準は元々データエンジニアリング領域を想定しており、新規ソースがどの帯に落ちるかは未知。初回実行後のスコア分布を見て、閾値と `profile.md` の基準の双方を再検討する。**閾値5と profile.md は「要合意」項目のため、実測を提示した上で運用者が判断する**。**2026-08-04 決定: 閾値は据え置き、`profile.md` の基準を書き直した。** 500件の分布（`0:102 1:15 2:89 3:76 4:66 5:6 6:45 7:34 8:65 9:2`）を `reason` ごと精査したところ、**低評価の主因は記事の質ではなく基準の設計**だった。(1) **129件が「関心領域外」で3点以下** — 基準が「データ基盤以外は3以下」と指示しており、処理系・型システムの記事を読むために追加した情報源（`scattered-thoughts` は186件中78件が0点）を基準自体が排除していた。(2) **5点に谷（4:66 → 5:6 → 6:45）** — 「判断材料が乏しい場合は 4〜5 を基準にする」という文言が**閾値ちょうどに曖昧なものを置いて**いた。対処として対象領域を「ソフトウェアエンジニアリング全般」へ広げ、材料不足は 0〜1 とした。**閾値の妥当性は書き直し後の分布を見てから再判断する**（領域が広がったぶん投入件数は増える見込み）。**2026-08-05 実測: 閾値5を据え置く。** 新基準で982件を評価した結果、閾値ごとの投入見込みは次のとおり（定常状態の新着を週50件と仮定した換算）。閾値5 → 週16件（率32%）/ 閾値6 → 週14件（28%）/ 閾値7 → 週10件（19%）。**閾値6にしても週2件しか減らない** — `final_score == 5` が43件（全体の4%）しかないため。その43件は「素点4 + weight+1」のハウツー記事が中心で、基準が言う「書き手の判断が残っている文章」からは外れる。**据え置いた理由:** 累積554件の投入は過去記事の総ざらいによるもので、定常状態の週16件は適量の可能性がある。**次の判断材料:** 定常状態の週次実行を3回以上観測してから再検討する。閾値7は要求定義の「週20本」想定を下回るが、実際に読める量としては現実的かもしれない（その場合 `#hot`（7以上）の定義と重なるため再定義が要る）。**2026-08-30 解決: 閾値5を確定する（据え置き）。** 待っていた定常状態の週次実行3回（08-09 / 08-16 / 08-23、新着計322件）を観測した。閾値ごとの週平均投入件数は **閾値4 → 23.0件（率21.4%）/ 閾値5 → 17.3件（16.1%）/ 閾値6 → 14.3件（13.4%）/ 閾値7 → 7.0件（6.5%）**（週内訳は 08-09: 13件 / 08-16: 16件 / 08-23: 23件）。**確定の理由:** (1) 閾値5の週17.3件が要求定義の「週20本」想定にほぼ一致する。閾値7は週7.0件まで落ちて想定を大きく割るため採れない。(2) 閾値6との差は週3件しかなく、動かす利得が小さい。(3) **境界帯（`final_score == 5`）の9件を実際に読んだところ、cybozu-tech『govulncheckで脆弱性スキャナーの偽陽性を削減する』、aws-database『Understand memory management in Amazon RDS for PostgreSQL』等、基準に合致する記事が並んでいた** — 閾値を上げるとこれらを取りこぼす。**08-05 時点の記述からの訂正:** 当時「`final_score == 5` は素点4 + weight+1 のハウツー記事が中心」と記録したが、これは総ざらい期のデータに基づく。**定常状態の9件は全て「素点5 + weight 0」であり、weight による押し上げではない。** 据え置きの結論は変わらないが、根拠は「境界帯が weight で嵩上げされた低質記事だから動かしても影響が小さい」ではなく「境界帯が素の実力で5点の妥当な記事だから落としてはいけない」に置き換わる。**再見直し条件:** 週次の投入件数が4回連続で10件未満または30件超となった場合、または `profile.md` の基準を再度書き直した場合 |
...
| TASK-081 | `OQ` | 評価の `usage`（トークン数）を state に記録するか決める | [SPEC-003 §OQ-008](03_specs/SPEC-003_entry_evaluation.md#11-未解決事項-open-questions) | t_inoue | 2026-08-23 | `wontfix` | 記録しない場合、コストの事後検証は実行サマリのみに依存する。TASK-041（コスト算出方法）と一体で判断。**2026-08-03 決定: 記録しない。** 事後に別単価で再計算できる利点はあるが、追記専用 JSONL が毎週太り続けるコストに見合わない（週あたり数十〜200行が積み上がる）。コスト検証は実行サマリと `runs.jsonl` で足りる |
...
| TASK-103 | `OQ` | 閾値 5 の妥当性と Databricks 系記事の評価の厳しさを実測で検証する | 実地 dry-run（2026-08-01） | t_inoue | 2026-09-30 | `resolved` | 2026-08-01 の3フィード40件で、**databricks-blog の投入3件がすべて補正後 5 ちょうど**（素点4 + weight+1）。weight がなければ全件落ちていた。また補正後 4 が4件あり「あと1点」の帯に溜まっている。基準が Databricks の記事に厳しすぎる可能性と、閾値5の妥当性の両方を、13フィード全件（約198件）の分布で判断する。R-006 の事後検証そのもの。**判断材料が40件では不足**。**2026-08-05 解決: 閾値5は据え置き、`databricks-blog` の低評価は妥当と判断した。** 累積1,680件（うち新基準982件）の実測による。(1) **閾値**: 閾値6にしても週2件しか減らない（`final_score == 5` が43件・全体の4%）ため、変更する意味が薄い。詳細は TASK-035。(2) **databricks-blog**: 16件すべて3点以下・投入0件だが、内容を確認すると「AI-forward healthcare organization」「Agentic media buying」といった**製品宣伝色の濃い記事**であり、基準どおりに落ちている。基準が厳しすぎるのではなく、この情報源の記事の性質が対象と合っていない。**投入0件が続くようなら情報源からの削除を検討する**（TASK-105 と同型の判断） |
...
| TASK-106 | `OQ` | 評価の全件失敗を運用者が識別できることを F 層の AC として起こすか | [TASK-101](#一覧) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **F-001 AC-017b を新設した。** `EVALUATE_ALL_FAILED` に対応する AC が上位になく、SPEC-005 §5 が観察可能な振る舞いを単独で決めている状態だった。**`FETCH_ALL_FAILED` 側は AC-017a が既に担保しており**（TASK-072 で新設済み）、穴は評価段階だけだった。AC-017b は「**実際に評価を試みた全件**」と限定する — 材料がなくスキップした記事（AC-023a）を分母に含めると全滅を検知し損ねるため（TASK-108 で実際に踏んだ）。SPEC-005 の T-023〜T-025 の対応 AC 列も `§5` から `F-001 AC-017b` へ付け替えた |
...
| TASK-111 | `IMPL` | `vutr-substack` が GitHub Actions から HTTP 403 になる件の扱いを決める | 実地の Actions 実行（2026-08-01） | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **情報源から削除した。** 原因は Substack の**データセンター IP ブロック**（Cloudflare 配下）。**User-Agent は原因ではない** — ローカルからは 現行UA / ブラウザUA / UAなし のいずれも 200 であり、フィード自体も正常に配信されている。Actions のランナーからのみ 403 になる。週次実行では毎回失敗し、**サマリの失敗行に埋もれて本当の障害を隠す**ため外した。`blef-fr`（TASK-027）と同じく削除理由を `feeds.yaml` にコメントとして残しており、実行環境が変われば（セルフホストランナー等）復活させられる。**採択率45%の情報源が失われる**ため補充は要検討 → TASK-114 |
| TASK-112 | `IMPL` | 状態ブランチから記録を読み戻す（冪等性の欠落） | 実地の Actions 実行（2026-08-01） | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: **本番実行で発覚。冪等性 R-002 が CI で成立していなかった。** 書き込みは状態ブランチ（`persist_state`）なのに、読み込みは CWD の `state.jsonl` という**非対称**があった。CI は毎回まっさらな checkout のためファイルが存在せず、**2回目の実行でも「新規10件・初回」と判定**された（同じ記事を毎週再評価し、閾値を超えれば重複投入する）。ローカルでは前回実行がファイルを残すため表面化しなかった。`persist.load_persisted()` を追加し `_Store` が起動時に読み戻す |
| TASK-113 | `IMPL` | 増分の切り出しをバイト単位で行う | [TASK-112](#一覧) | t_inoue | 2026-08-01 | `resolved` | 2026-08-01: TASK-112 の修正中に検出。`persist_state` は**追記**のため読み戻した全文を渡すと二重になる。増分だけを送る必要があるが、位置を `st_size`（**バイト数**）で持ちながら `str` の**文字数**スライスで切っていた。**日本語のタイトル・理由を含む行でズレ、行の途中から切り出して壊れた JSON を push していた**（読み戻し時に捨てられ記録が消える）。ASCII のみのテストでは一致してしまい検出できない。マルチバイトを含む回帰テストを追加し、変異で検出を確認した |
| TASK-114 | `OQ` | `vutr-substack` の削除で失われた情報源を補充するか判断する | [TASK-111](#一覧) | t_inoue | 2026-09-30 | `open` | 2026-08-01: 削除した `vutr-substack` は採択率45%（20件中9件）で、個人ブログとして基準に合致しやすい側だった。**ローカルからの疎通のみ**確認した候補: `engineering.atspotify.com/feed`（200）、`materializedview.io/feed`（200）、`www.confluent.io/blog/rss.xml`（301）、`blog.det.life/feed`（接続不可）、`eng.uber.com/feed/`（404）。**Actions からの疎通は未確認**であり、vutr と同じ罠を踏む可能性がある。追加時は weight 0 から始めて実測で調整する |
| TASK-115 | `OQ` | トリアージ判定を多軸化するか判断する（検討済み・当面見送り） | 実地の分布分析（2026-08-04） | t_inoue | 2026-10-31 | `wontfix` | 2026-08-04: 単一スコア（0-10）を `unscorable` / `scope` / `judgment` / `mechanism` の多軸へ分解する案を検討し、**当面見送った**。動機は「0点が『対象外』『材料不足』『低評価』を同じ値に潰しており、落ちた理由が読めない」こと。**見送りの理由:** (1) 実データの精査で、低評価の主因が基準の文言（領域の限定と「材料が乏しければ4〜5」）だと判明し、`profile.md` の書き直しだけで対処できる見込みが立った。(2) `scope` は情報源ごとの `weight` と重複し、投入先が単一コレクションのため配架の実体がない。(3) `judgment_markers` は合成規則が消費せず検証不能で、LLM が後付けで理由を並べる余地が大きい。(4) `mechanism` は合成表の1セルしか変えない。(5) 利用者1人・週数十件の規模に対して複雑すぎる（YAGNI）。**再検討の条件:** 書き直し後の分布でも落ちた理由が読めない、または「主張は弱いが仕組みの解説として価値がある」記事を体系的に取りこぼしていることが実測で確認された場合。**2026-08-05 実測: 書き直しだけで所期の効果が出たため、多軸化の必要性はさらに下がった。** (1) 「関心領域外」を理由とする3点以下が **129件 → 21件**（残る21件は DJ・ゲームジャム参加報告・圏論そのものの解説で、落ちるのが正しい）。(2) 5点の谷が解消（`4:66 → 5:6 → 6:45` が `4:22 → 5:16 → 6:58` へ）。(3) **weight 依存が 29% → 6%** — 情報源ごとの下駄がなくても基準本体で拾えるようになった。多軸化が解こうとしていた「落ちた理由が読めない」問題は、基準の文言で解消できることが実証された |
| TASK-116 | `IMPL` | `preferred-tech` のフィード URL を修正する（現 URL は HTTP 404） | 実測（2026-08-31） | t_inoue | 2026-09-30 | `open` | 2026-08-31: `https://tech.preferred.jp/ja/blog/feed/` が **HTTP 404**（Next.js のエラー HTML が返る）。08-30 の実行の `source_errors` にも記録された。`feeds.yaml` は `verified: true` のままで実態と乖離している。**削除ではなく URL 修正で復帰できる可能性が高い**（サイト自体は稼働中）ため、正しいフィード URL を調べて差し替える。復帰後は weight +1 の妥当性も TASK-118 とあわせて確認する |
| TASK-117 | `OQ` | `paulgraham` / `conal` を情報源として維持するか判断する | 実測（2026-08-31） | t_inoue | 2026-09-30 | `open` | 2026-08-31: 両者とも `feeds.yaml` の URL が `http://` で、パイプラインが「https 以外の宛先には接続しません」として弾いている。**9実行すべてで取得0件**。さらに **https 版が存在しない**（`https://` へ差し替えても `ConnectError`）ため、**URL の書き換えでは解決しない**。TASK-027（`blef-fr`）と同型で、削除して理由をコメントに残す判断の対象。放置すると毎週 `source_errors` に2行出続け、TASK-111 で問題視した「本当の障害が埋もれる」状態を作る。**削除を保留する場合も `verified: true` は実態と乖離しているため下げる。** なお `paulgraham` は配信219件を保持しており、仮に将来 https 化されると一括流入する（→ TASK-119） |
| TASK-118 | `OQ` | weight 機構が機能していない件の扱いを決める | 実測（2026-08-31） | t_inoue | 2026-09-01 | `resolved` | 2026-08-31: 新基準1,304件で weight が投入可否を**変えた件数は20件（1.5%）**、**押し下げて落とした件数は0件**。weight を設定しているのに影響0件のソースが18件ある（`cloudflare +1` / `wadler +1` / `mametter +1` / `preferred-tech +1` 等）。特に `aws-machine-learning` の `weight: -1`（「量が多く告知比率が高いため事前減点」）は**素点の最高が4のため一度も結果を変えていない** — 減点しなくても全件落ちる。TASK-035 で確認した「閾値も実用域では週3件しか動かない」と合わせると、**投入内容を動かすレバーは実質 `profile.md` の基準だけ**であり、ADR-003 OQ-002 の結論を補強する。weight を廃止するか、効く値域（±2 以上）に変えるか、`profile.md` 側へ統合するかを決める。**ADR 化の要否も含めて判断する**。**2026-09-01 解決: weight を全廃した。** 追加実測で、定常状態の4回（08-09 / 16 / 23 / 30）では**weight が投入可否を変えた件数が0件**であることを確認した（押し上げの20件はすべて 08-04 の総ざらい期）。08-30 の投入133件のうち111件が weight +1 のソースだったが、**133件すべてが素点だけで閾値以上**であり、weight がなければ落ちていた件数は0件。個人ブログが多く投入されるのは weight ではなく素点が高いためで（平均素点は weight +1 が 6.08、weight 0 が 4.13）、重みは屋上屋になっていた。**全廃した場合の投入件数は定常状態4回で 185件 → 185件と変化しない**ため、廃止のリスクはない。`feeds.yaml` から `weight` を削除（58件）、`config.py` は常に 0、`scoring.adjust(score, weight)` を `adjusted_score(score)` へ置換（`should_ingest` の引数名との衝突を避けて命名）。**state の読み取りからは weight を除いていない** — SPEC-002 §5 により `final_score` は保存値を信用せず `score + weight` で再計算されるため、除くと既存1,326行の `final_score` が遡って変わり R-006 の事後検証が壊れる。既存2,224行を現行コードで読み直し `final_score` が変化しないことを確認済み。**ADR 化は見送った** — 「効かない機構を外す」だけで設計上の選択肢が拮抗していないため。**SPEC 層への反映は別途必要**（→ TASK-120） |
| TASK-119 | `OQ` | 1ソースの一括流入がスコア分布を占有する件への対処を決める | 実測（2026-08-31） | t_inoue | 2026-08-31 | `resolved` | 2026-08-31: 08-30 の実行で **`danluu` が初めて取得成功し、過去記事128件が一斉に新着となった**（それまで毎回「応答が上限（10 MB）を超えました」で0件。フィードが 6.15 MB へ縮み上限内に収まった）。その結果、**新着222件中128件・投入133件中108件（81%）が単一ソース**となり、採択率が 59.9%（直近3週は 13.4/14.4/20.2%）、素点8の比率が 2.8% → 30.6% へ跳ねた。**評価自体は妥当**（`danluu` は weight +1 の著名な技術ブログで、内容も基準に合致）であり `danluu` は消化済み（残り0件）だが、**同じ構造のリスクが `paulgraham` の219件に残る**（TASK-117。`danluu` の1.7倍）。現状 `DEFAULT_EVALUATION_LIMIT = 500` の**全体上限のみで、ソースごとの上限がない**ため、1ソースが枠と週次の分布を占有しうる。対処案: (a) ソースごとの新着上限を設ける (b) 初回取得時のみ件数を絞る (c) 対処せず一過性として許容する。**選定にあたり F-001 AC-025a（新規優先で枠を埋める）との整合を確認する**。**2026-08-31 解決: (a) ソースごとの新着上限を採用し実装した。** `DEFAULT_PER_SOURCE_LIMIT = 30` を `domain/state.py` に定数化し、`limit_per_source()` で**新規記事のみ**を情報源ごとに絞る。**30 の根拠:** 定常状態の新着は週97〜114件、最大ソースの `simonwillison` でも週22.3件（TASK-035 の実測）であり、30 は平常時の最大を上回るため**通常の運用では発動しない**。発動するのは一括流入時のみ。**AC-025a との整合:** 絞る対象を新規のみに限り再評価対象には適用しないため、「新規優先で枠を埋める」順序は保たれる（`limit_per_source` → `select_evaluation_targets` の順に適用）。**08-30 の実データで検証:** danluu は128件 → 30件に収まり、残り98件は4週かけて消化される。テスト8件を追加（計425件、カバレッジ 97.03%）。**残る観察点:** 上限30でも 08-30 の採択率は 37.9% までしか下がらない。danluu を除いた素の採択率が 26.6% と直近3週（13.4〜20.2%）よりやや高いためで、`zozo-tech` 7/9・`m-hiyama` 3/7 といった小 n のぶれが要因。系統的な変化ではないと判断したが、**来週の実行で 20% 前後に戻るかを確認する**。**2026-08-31 追記: 上限30が「最新30件を残す」挙動になっていることを確認した。** 公開日フィルタ（古い記事の足切り）を入れるか検討したが、**不要と判断した**。`limit_per_source` は取得順を保って先頭を残し（SPEC-001 §4）、**58ソース中、日付順を検査できた53件のうち52件がフィードを新しい順に配信している**ため、「先頭30件」が実質「最新30件」になる。`danluu` で実測すると、今週評価される先頭30件は 10〜1775日前、持ち越される98件は 1778〜5274日前であり、**押し出されるのは常に古い側で最新記事は必ず当週に入る**。公開日で足切りすると、`danluu` の108件投入のような過去の良記事を捨てることになり、また `paulgraham` の219件は**公開日が取得できない**ため F-001 AC-026（日時欠落でも評価対象に含める）との整合で例外が増える。**残る前提:** 例外は `martinfowler` の1件のみで、これは日付降順で配信していない。現在30件配信のため上限に掛からず実害はないが、**配信件数が30を超えると「先頭30件＝最新30件」が成り立たなくなる**。同種のソースを追加する場合はこの前提を再確認する |
| TASK-120 | `DOC` | weight 廃止を SPEC 層と用語集へ反映する | [TASK-118](#一覧) | t_inoue | 2026-09-30 | `open` | 2026-09-01: 実装は TASK-118 で完了したが、**SPEC 層の記述が weight 前提のまま残っている**。対象: `SPEC-001 §4`（入力定義の `weight` 行）、`SPEC-002 §5`（`final_score = score + weight` の算出規則・バリデーション）、`SPEC-004 §4`（補正後スコアの値域注記・T-006 / T-019 / T-020 の観点）、`00_glossary.md`（「重み (weight)」の項）、`F-001 AC-005` / `REQ-F-004`（重みによる補正の要件そのもの）。**SPEC-002 の `weight` 読み取り規定は残す** — 既存行の `final_score` を保全するため実装でも残しており、記述を消すと実装と乖離する。**上位の REQ-F-004 を廃止扱いにするかは要判断**（要件の削除は F/SPEC より上位の変更にあたる）。3本とも `approved` 相当のため、変更後は `docs/05_guides/prompts/independent_review.md` を別セッションで回す |
...
| TASK-041 | `OQ` | 実行サマリの評価コストの算出方法を決める | [F-004 §OQ-003](02_features/F-004_execution_summary.md#8-未解決事項-open-questions) | t_inoue | 2026-08-23 | `resolved` | API 応答のトークン数から算出するか、件数からの概算に留めるか。REQ-NF-002a の検知手段。**2026-07-28 追記:** `usage` が得られない実装だと `evaluated > 0` でも `cost_usd` が 0.0 になりうる。その場合コスト超過検知が「$0.000 なのに評価 200 件」の形で静かに壊れるため、算出失敗を警告する規定の要否も一緒に決める（SPEC-006 OQ-001）。**2026-08-03 決定:** API 応答の `usage` から算出する（`domain/cost.py`）。単価はモデル固有の定数。算出失敗の警告も入れる。**実地の Actions 実行で「$0.000 なのに評価 150 件」が実際に発生**しており、予見どおりの壊れ方だった。あわせて実行内リトライで最後の試行分しか計上されない過少計上も修正（`evaluate.py`） |
...
| 2026-08-30 | t_inoue | **TASK-035（閾値の感度検証）を `resolved` に。閾値5を確定した。** 08-05 に「定常状態の週次実行を3回以上観測してから再検討する」として保留していた条件が、08-09 / 08-16 / 08-23 の3回（新着計322件）で揃ったため判断した。閾値ごとの週平均投入は 4→23.0件 / 5→17.3件 / 6→14.3件 / 7→7.0件で、**閾値5の週17.3件が要求定義の「週20本」想定にほぼ一致する**。閾値6との差は週3件にとどまり、境界帯（`final_score == 5`）の9件を実際に読むと基準に合致する記事が並んでいたため、上げると取りこぼしになる。あわせて **08-05 の「境界帯は素点4 + weight+1 のハウツー記事が中心」という記述を訂正**した — それは総ざらい期のデータで、定常状態の9件は全て素点5 + weight 0 であり weight による嵩上げではない（結論は不変だが据え置きの根拠が入れ替わる）。ADR-003 OQ-002 も `resolved` とし、**モデルでも閾値でもなく `profile.md` の基準が最も効くレバーだった**ことを結論として記録した（閾値は実用域で週3件しか動かないのに対し、08-04 の基準書き直しは投入率を 11% → 13〜20% へ動かした）。次の関心は情報源側（投入0件が7ソース → TASK-105 / TASK-114） |
| 2026-08-31 | t_inoue | **08-30 の実行でスコア分布が崩れた件を調査し、原因を特定して恒久対処した。** 採択率が 59.9%（直近3週は 13.4〜20.2%）、素点8の比率が 2.8% → 30.6% へ跳ねた原因は、**`danluu` が初めて取得に成功し過去記事128件が一斉に新着となったこと**（10 MB 超で毎回失敗していたフィードが 6.15 MB へ縮んだ）。投入133件中108件（81%）が単一ソースだった。**評価自体は誤判定ではなく**（内容も基準に合致）、`danluu` の残りは0件で消化済みだが、同型のリスクが `paulgraham` の219件に残るため `DEFAULT_PER_SOURCE_LIMIT = 30` を導入した（TASK-119、テスト8件追加で計425件）。あわせて全58ソースの滞留を走査し、`paulgraham` / `conal` が `http://` のため**恒久的に取得不能**（https 版も存在しない）であること、`preferred-tech` が **HTTP 404** であることを確認。また新基準1,304件で **weight が投入可否を変えたのは20件（1.5%）・押し下げは0件**で、`aws-machine-learning` の `weight: -1` は素点最高が4のため一度も結果を変えていないことが判明した。TASK-116〜119 を登録 |
| 2026-09-01 | t_inoue | **情報源ごとの重み付け（weight）を全廃した**（TASK-118）。定常状態の4回（08-09 / 16 / 23 / 30）で **weight が投入可否を変えた件数は0件**。08-30 の投入133件は111件が weight +1 のソースだったが、**全133件が素点だけで閾値以上**であり weight がなければ落ちていた件数は0件だった。個人ブログが多いのは重みではなく素点が高いためで（平均素点 weight+1: 6.08 / weight 0: 4.13）、全廃しても定常状態4回の投入は 185件 → 185件と変わらない。`feeds.yaml` から weight を削除し、`adjust(score, weight)` を `adjusted_score(score)` へ置換。**state の読み取りからは weight を残した** — `final_score` は保存値を信用せず再計算される仕様のため、除くと既存1,326行の値が遡って変わり R-006 の事後検証が壊れる（2,224行で不変を確認済み）。TASK-035 の「閾値も実用域では週3件しか動かない」と合わせ、**投入内容を動かすレバーは `profile.md` の基準だけ**という理解が確定した。SPEC 層への反映は TASK-120 として起票 |
