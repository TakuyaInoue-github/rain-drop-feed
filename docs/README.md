...
| TASK-118 | `OQ` | weight 機構が機能していない件の扱いを決める | 実測（2026-08-31） | t_inoue | 2026-09-30 | `open` | 2026-08-31: 新基準1,304件で weight が投入可否を**変えた件数は20件（1.5%）**、**押し下げて落とした件数は0件**。weight を設定しているのに影響0件のソースが18件ある（`cloudflare +1` / `wadler +1` / `mametter +1` / `preferred-tech +1` 等）。特に `aws-machine-learning` の `weight: -1`（「量が多く告知比率が高いため事前減点」）は**素点の最高が4のため一度も結果を変えていない** — 減点しなくても全件落ちる。TASK-035 で確認した「閾値も実用域では週3件しか動かない」と合わせると、**投入内容を動かすレバーは実質 `profile.md` の基準だけ**であり、ADR-003 OQ-002 の結論を補強する。weight を廃止するか、効く値域（±2 以上）に変えるか、`profile.md` 側へ統合するかを決める。**ADR 化の要否も含めて判断する** |
