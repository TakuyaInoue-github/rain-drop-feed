...
| TASK-101 | `DOC` | `EVALUATE_ALL_FAILED = 6` を SPEC-005 §5 と SPEC-003 に反映する | [TASK-099](#一覧) | t_inoue | 2026-08-23 | `open` | SPEC-005 は終了コードの**正典**であり、実装が先行している状態。§5 の表・優先順位表・dry-run 節の3箇所と、SPEC-003 の評価失敗の扱いに追記する。あわせて F-002 に対応する AC がない点も確認する（F-001 AC-014 が投入側にしかない） |
